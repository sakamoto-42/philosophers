...
/*   Updated: 2025/03/27 16:08:47 by juduchar         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	ft_check_all_philos_ate_enough(t_monitoring *monitoring)
{
	...
	if (!ft_mutex_is_simulation_finished(monitoring))
	{
		...
		printf("all (%d) philos ate enough (%d) meals\n",
			monitoring->data->number_of_philosophers,
			monitoring->data->meals_required);
	...
	while (i < monitoring->data->number_of_philosophers)
	{
		...
		if (time_in_ms - last_meal_time > monitoring->data->time_to_die)
		{
			...
			printf("[%ldms] philo %d %s",
				ft_get_time_in_ms() - monitoring->data->start_time_in_ms,
